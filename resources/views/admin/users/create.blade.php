    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-xl shadow-sm">
            <div class="px-8 py-6 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Complete los datos para crear un nuevo usuario del panel.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" class="px-8 py-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}"
                               ...
                               placeholder="usuario@otium.com.bo">
                        @error('email')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <select id="role" name="role"
                        ...
                        <option value="editor" @selected(old('role') === 'editor')>Editor — Ver y marcar mensajes</option>
                        <option value="admin" @selected(old('role') === 'admin')>Admin — Gestionar mensajes y ver usuarios</option>
                        <option value="super_admin" @selected(old('role') === 'super_admin')>Super Admin — Acceso completo</option>
                    </select>
                    @error('role')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
